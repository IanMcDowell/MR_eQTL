...
unscaled_var_beta_MR = np.load(args.unscaled_var_beta_MR)

# create dictionary of z->x associations
z_to_x_top_beta = {z:(beta_xz,x) for z, x, beta_xz in zip(z_to_x_top_beta[0],z_to_x_top_beta[1],z_to_x_top_beta[2])}

# prepare expression for genes x and genes y
X_expr_dict = {gene:np.array(X.ix[gene]) for gene in list(X.index)}
Y_expr_dict = {gene:np.array(Y.ix[gene]) for gene in list(Y.index)}

# create dictionaries for rapid access to cells of unscaled_var_beta_MR
row_dict = {snp:i for i,snp in enumerate(list(Z.index))}
col_dict = {gene:j for j,gene in enumerate(list(X.index))}

# v provided:
v = 3
n = X.shape[1]

assert(len(set(z_to_x_top_beta.keys()) - set(row_dict.keys())) == 0)
